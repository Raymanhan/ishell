...
**[下载 iShell v1.1.20](https://github.com/Raymanhan/ishell/releases/tag/v1.1.20)**
...
- **SFTP 文件浏览器**：支持树形/多列视图、上传、下载、新建文件夹、重命名、删除、右键菜单、软链接识别、终端目录跳转、小文件远程编辑和独立日志滚动查看。
- **实时主机监控**：展示 CPU、GPU、显存、内存、Swap、磁盘、负载、进程排行和网络吞吐，并支持一键复制主机/IP。
...
- 右键标签页可以打开标签级操作菜单。
- 顶部栏可设置为常驻显示，或在鼠标离开后自动隐藏、移到窗口顶端时再显示。
- 终端字号可在设置中调整。
...
- 在树形详情视图和多列视图之间切换。
- 上传文件或文件夹到当前目录，支持拖放目录；远端存在同名文件夹时会先确认是否完整替换，替换不会合并，并会删除远端独有内容。
- 下载选中的文件；新加入的传输任务会显示在队列顶部，方便查看最新进度。
- 新建文件夹、重命名条目、删除一个或多个条目。
- 软链接会显示专用图标，删除确认会说明只删除链接本身。
- 可从 SFTP 右键菜单将当前终端跳转到选中的远程目录。
- 打开小型文本文件，在内置编辑器中修改并保存回远程主机。
- 对选中文件打开实时滚动查看窗口，持续跟随远端日志新增内容。
- 拖动选择文本超出编辑器左右边界时，会自动横向滚动并继续扩展选择范围。
- 使用路径栏直接跳转到指定目录。
...
- 在 SFTP 文件右键菜单中选择实时滚动查看，针对当前主机上的文件打开独立窗口。
- 查看窗口不会占用主工作台，适合跟随应用日志、部署日志或系统日志。
- 可设置初始加载的尾部行数，并在窗口内重新应用。
- 支持暂停跟随、回到底部继续跟随、清空当前输出和重新启动滚动查看。
- 窗口关闭时会停止对应的远端 tail 进程，避免后台任务残留。
...
src-tauri/src/
  ...
  tail_monitor.rs   远程文件实时滚动查看进程注册表
...
- `v1.1.20` 新增远程日志独立实时滚动查看窗口，改进远程编辑器垂直拖拽滚动、SFTP 详情列宽调整，并优化柔白主题和新增服务器弹窗。
- `v1.1.19` 扩展实时监控面板，新增 GPU/显存指标，并加入按 CPU 与内存排序的实时 Top 5 进程排行。
- `v1.1.18` 新增顶部栏自动隐藏开关，并将新加入的上传/下载任务显示在队列顶部，方便追踪最新传输进度。
- `v1.1.17` 改进远程文件编辑体验，拖动选择文本超出编辑器左右边界时可自动横向滚动并继续选择。
- `v1.1.16` 将 SFTP 文件与文件夹上传整合到工具栏和右键子菜单，并完善菜单关闭与焦点恢复行为。
- `v1.1.15` 新增事务式文件夹上传与目录拖放，提供完整替换确认、原子提交、失败回滚，并强化两套 SSH 后端的取消与重试行为。
- `v1.1.14` 修复 Linux CI 对 macOS 专用窗口重开处理的 lint 检查问题。
- `v1.1.13` 增强 SSH/SFTP 稳定性，补充连接超时、主机密钥持久化、独立上传连接、传输保护，并在 CI 中加入 Rust 检查与测试。
- `v1.1.12` 将历史命令候选快捷键调整为 Alt，并优化候选列表为空时的标题显示。
- `v1.1.11` 为 Windows 终端补充剪贴板复制和粘贴快捷键支持。
- `v1.1.10` Windows 窗口改用 Mica 背景效果，减少移动或缩放窗口时的卡顿。
- `v1.1.9` 优化文件夹与归档下载的未知大小进度显示，改为实时传输字节数和动态进度条。
- `v1.1.8` 将大文件与文件夹下载切换到独立 SSH 连接，避免下载流量挤占交互式终端连接。
- `v1.1.7` 新增 SFTP 远程文件夹下载，可从右键菜单选择打包下载 `.tar.gz` 或直接下载，并优化下载队列中的未知大小进度显示。
